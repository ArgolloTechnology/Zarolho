...
int eyeStyle = 1; // 0 = Normal, 1 = Coração, 2 = Espiral
...
void drawEyes(){
  ...
  if (eyeStyle == 0) display.fillRoundRect(eyeLx, eyeLy, eyeLwidthCurrent, eyeLheightCurrent, eyeLborderRadiusCurrent, MAINCOLOR); // left eye
  if (eyeStyle == 1) {  // Coração
    uint8_t heartX = eyeLx;
    uint8_t heartY = eyeLy;
    uint8_t size = 16;  // Tamanho do coração

    // Desenha um coração usando pequenos círculos e um triângulo
    display.fillCircle(heartX + eyeLwidthCurrent/4, heartY + eyeLheightCurrent/2, eyeLwidthCurrent / 4, WHITE);
    display.fillCircle(heartX + 3*(eyeLwidthCurrent/4), heartY + eyeLheightCurrent/2, eyeLwidthCurrent / 4, WHITE);
    display.fillTriangle(heartX - 1, heartY + 1 + eyeLheightCurrent/2, heartX - 1 + eyeLwidthCurrent, heartY + 1 + eyeLheightCurrent/2, heartX - 1 + eyeLwidthCurrent/2, heartY + eyeLheightCurrent*1.5, WHITE);
  }
  ...
  if (!cyclops){
    ...
    heartX = eyeRx;
    heartY = eyeRy;
    display.fillCircle(heartX - 3, heartY - 3, size / 2, WHITE);
    display.fillCircle(heartX + 3, heartY - 3, size / 2, WHITE);
    display.fillTriangle(heartX - size, heartY, heartX + size, heartY, heartX, heartY + size, WHITE);
    */
  }
  if(eyeStyle == 0){
